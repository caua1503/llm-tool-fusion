...
def process_tool_calls(
    ...
    Compatível com qualquer framework (OpenAI, LangChain, Ollama, etc.) desde que forneça uma função de chamada
    (llm_call_fn).
    ...
    Args:
        ...
        llm_call_fn (obrigatorio): função que faz a chamada ao modelo (ex: lambda model, messages, tools: ...)
        ...
        while True:
            ...
            if chain_count > max_chained_calls:
                ...
                messages.append({
                    "role": "system",
                    "content": (
                        f"The maximum number of chained calls ({max_chained_calls}) has been reached. "
            ...
            if use_async_poll and async_poll_list:
                ...
                async_results = asyncio.run(
                    _poll_fuction_async(
                        avaliable_tools=available_tools, list_tasks=async_poll_list, framework=framework
                    )
                )
...
async def process_tool_calls_async(
    ...
    """
    Processa tool_calls de uma resposta de LLM, executando as ferramentas necessárias e atualizando as mensagens.
    Compatível com qualquer framework (OpenAI, LangChain, Ollama, etc.)
    desde que forneça uma função de chamada (llm_call_fn).
    ...
    Args:
        ...
        llm_call_fn: função que faz a chamada ao modelo (ex: lambda model, messages, tools: ...)
